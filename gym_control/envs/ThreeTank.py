...
class ThreeTank(gym.Env):
    ...
    def q13(self, mu13=0.5, sn=0.00005):
        return np.sign(self.x[0] - self.x[2]) * np.sqrt(2 * const.g * np.sqrt(self.x[0] - self.x[2])) * mu13 * sn

    def q32(self, mu32=0.5, sn=0.00005):
        return np.sign(self.x[2] - self.x[1]) * np.sqrt(2 * const.g * np.sqrt(self.x[2] - self.x[1])) * mu32 * sn
    ...
    def step(self, action: np.ndarray) -> Tuple[Any, float, bool, Dict]:  # Obs, Reward, Done, Info
        ...
        return np.append(
            obs * (1. + np.random.normal(loc=np.zeros(2, ), scale=np.array([0.00, 0.07]))) if self.noise else obs,
            np.array([0., 0.])), reward, win, {
                   'u': u,
                   'x': self.x,
                   'dx': dx
               }
...
class AdversarialThreeTank(gym.Env):
    def __init__(self, compromise_actuation_prob: float, compromise_observation_prob: float) -> None:
        ...
        self.env = gym.make('ThreeTank-v0')
