...
class UniformDefender(BaseDefender):
    ...
    def predict(self, observation, state=None, mask=None, deterministic=True):
        ...
        for i in range(self.m):
            if observation[i * 5 + 0] == 1:  # and obs[i * 5 + 2] > 0:
                targets.append(i)
...
class PCPDefender(BaseDefender):
    ...
    def predict(self, observation, state=None, mask=None, deterministic=True):
        targets = []
        for i in range(self.m):
            if observation[i * 5 + 2] >= self.pi:  # 0 ~> up, 1 ~> time_to_up, 2 ~> observed_progress, 3 ~> time_since_last_probe, 4 ~> time_since_last_reimage
                targets.append(i)

            if 1 <= observation[i * 5 + 2] and observation[i * 5 + 3] > self.p:
                targets.append(i)
...
class ControlThresholdDefender(BaseDefender):
    ...
    def predict(self, observation, state=None, mask=None, deterministic=True):
        ...
        for i in range(self.m):  # 0 ~> up, 1 ~> time_to_up, 2 ~> observed_progress, 3 ~> time_since_last_probe, 4 ~> time_since_last_reimage
            if observation[i * 5 + 2] != 0:
                prob = (1 - math.exp(-self.alpha * (observation[i * 5 + 2] + 1)))
...
class MaxProbeDefender(BaseDefender):
    ...
    def predict(self, observation, state=None, mask=None, deterministic=True):
        ...
        for i in range(self.m):  # 0 -> up / 1 -> time to up / 2 -> progress / 3 -> control
            ...
            if observation[i * 5 + 0] == 1:
                if observation[i * 5 + 2] == max:
                    max_count += 1
                    if random.random() < 1 / max_count:
                        index = i
                if observation[i * 5 + 2] > max:
                    index = i
                    max = observation[i * 5 + 2]
                    max_count = 1
